...
# 10-feb-2017/sklar
#
# This program demonstrates the idea of "online" training for a neural
# network.
#
# The online training mode involves seeing one training example
# (instance) at a time and adjusting the network weights after each
# example, until the error rate is low or for a fixed number of
# training cycles. This mode of training is appropriate for situations
# where all the training data is not available prior to training, for
# example, when a robot is learning as it explores an environment. In
# contrast, "batch" training involves seeing all examples from the
# training set before adjusting the network weights. This mode of
# training is appropriate for situations when all the training data is
# available to the learner prior to beginning training.
#
...
#  $ python train-online.py train.dat valid.dat eval.dat weights.dat training.log
# where: 
...
#-loop through the raw data, and train on one example at a time
gen = 0
recnum = 0
